...
function ensure_tables(PDO $pdo){
  ...
  $pdo->exec("CREATE TABLE IF NOT EXISTS project_daily_log_images (
    id INT AUTO_INCREMENT PRIMARY KEY,
    daily_log_id INT NOT NULL,
  ...
  if (!has_column($pdo,'project_daily_log_images','file_name')) {
    try { $pdo->exec("ALTER TABLE project_daily_log_images ADD COLUMN file_name VARCHAR(255) NULL AFTER file_path"); } catch(Throwable $e){ /* ignore */ }
  }
  ...
  try{
    ...
    if ($action==='create' || $action==='update'){
      ...
      if ($action==='create'){
        $st=$pdo->prepare("INSERT INTO project_daily_logs (project_id,code,entry_date,name,approval_group_id,status,weather_morning,weather_afternoon,weather_evening,weather_night,site_cleanliness,labor_safety,work_detail,created_by)
                           VALUES (?,?,?,?,?,'pending',?,?,?,?,?,?,?,?)");
        $st->execute([$projectId,$code,$date,$name,$group,$wm,$wa,$we,$wn,$clean,$safety,$detail,(int)($GLOBALS['userId']??0)]);
        ...
        notify_group($pdo,$projectId,$group,$dailyId,(int)($GLOBALS['userId']??0),$name,$date);
    ...
    elseif ($action==='get_log'){
      ...
      if (has_column($pdo,'project_daily_log_images','file_name')) {
        ...
        // fallback: derive file_name from path
 ...
 >
  ...
  <div class="dl-table-wrap">
    <table class="dl-table">
      ...
      <tbody id="dl-tbody">
        <?php
        $st=$pdo->prepare("SELECT * FROM project_daily_logs WHERE project_id=? ORDER BY entry_date DESC, id DESC");
        $st->execute([$projectId]);
        foreach($st->fetchAll(PDO::FETCH_ASSOC) as $r): ?>
          <tr class="dl-row" data-name="<?= htmlspecialchars($r['name']) ?>">
...
<!-- Assets hook (Font Awesome 5.15.4 assumed available globally) -->
  <link rel="stylesheet" href="../assets/css/project_tab_daily.css?v=<?php echo time(); ?>">
