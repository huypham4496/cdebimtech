...
// — Date context for calendar & export/send-report —
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['export_excel'])) {
        ...
        echo '<h3>Work Diary ' . DateTime::createFromFormat('!m', $month)->format('F') . " $year</h3>";
        echo '<table class="data"><tr><th>Date</th><th>Period</th><th>Content</th></tr>';
        foreach ($grouped as $d => $periods) {
            foreach (['morning','afternoon','evening'] as $prd) {
                if (!empty($periods[$prd])) {
                    echo '<tr>'
                       . '<td>' . htmlspecialchars($d) . '</td>'
                       . '<td>' . ucfirst($prd) . '</td>'
                       . '<td>' . nl2br(htmlspecialchars($periods[$prd])) . '</td>'
                       . '</tr>';
                }
            }
        }
    ...
    // === Send Monthly Report & Notifications ===
    if (isset($_POST['send_report'])) {
        $reportMonth = (int)($_POST['report_month'] ?? $month);
        $reportYear  = (int)($_POST['report_year']  ?? $year);

        // Query all entries for that month
        $rs = $pdo->prepare(
            "SELECT entry_date, period, content
               FROM work_diary_entries
              WHERE user_id = ? AND MONTH(entry_date)=? AND YEAR(entry_date)=?
        ...
        $rs->execute([$userId, $reportMonth, $reportYear]);
        $reportData = $rs->fetchAll();

        // Build simple text body
        $body = "Monthly Work Diary Report for {$userName} - {$reportMonth}/{$reportYear}\n\n";
        $curr = '';
        foreach ($reportData as $row) {
            if ($row['entry_date'] !== $curr) {
                $curr = $row['entry_date'];
                $body .= "Date: {$curr}\n";
            }
            $body .= ucfirst($row['period']) . ": " . $row['content'] . "\n";
        }
        // TODO: mail($to, $subject, $body);

        // Queue notifications
        if (!empty($_POST['notify_users'])) {
            $notifyDate = sprintf('%04d-%02d-01', $reportYear, $reportMonth);
            $nq = $pdo->prepare(
                "INSERT INTO notifications (sender_id, receiver_id, entry_date) VALUES (?, ?, ?)"
            );
            foreach ($_POST['notify_users'] as $rid) {
                $nq->execute([$userId, (int)$rid, $notifyDate]);
            }
        }

        $notifyMsg = "Monthly report sent and notifications queued.";
    }

    // === Save single-day Diary Entries ===
    if (isset($_POST['save_diary'])) {
        $up = $pdo->prepare(
            "REPLACE INTO work_diary_entries (user_id,entry_date,period,content) VALUES (?,?,?,?)"
        );
        $del = $pdo->prepare(
...
// — Load entries for $date & build calendar —
$f = $pdo->prepare("SELECT period,content FROM work_diary_entries WHERE user_id=? AND entry_date=?");
...
$monthName = ucfirst(strtolower($current->format('F')));
...
<div class="main-content">
  ...
  <div class="calendar-container card-block" id="calendar">
    ...
    <table class="calendar">
      <thead>
        <tr><?php foreach (['Mon','Tue','Wed','Thu','Fri','Sat','Sun'] as $d): ?>
          <th><?= $d ?></th>
        <?php endforeach; ?></tr>
      </thead>
      <tbody>
        <?php foreach ($weeks as $row): ?>
          <tr>
            <?php for ($d = 1; $d <= 7; $d++): 
              if ($row[$d]): 
                $ds  = sprintf('%04d-%02d-%02d', $month, $year, $row[$d]);
  ...
  <form method="post" class="entry-panel" id="entryPanel">
    <div class="card-block notify-panel">
      <div class="company-label">
        Company: <span><?= htmlspecialchars($company, ENT_QUOTES) ?></span>
      </div>
      <h4>Send Monthly Report</h4>
      ...
      <button name="send_report" class="btn-send-report">
    ...
    <!-- Actions -->
    <div class="actions">
      <button type="submit" name="save_diary" class="save"><i class="fas fa-save"></i> Save</button>
      <button type="submit" name="export_excel" class="export"><i class="fas fa-file-excel"></i> Export Excel</button>
    </div>
  </form>
</div>
